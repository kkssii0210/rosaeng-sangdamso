...
async function unusedPatchNotePath(inboxDir, slug) {
  const basePath = path.join(inboxDir, `${slug}.md`);
  ...
  for (let index = 2; index < 1000; index += 1) {
    const filePath = path.join(inboxDir, `${slug}-${index}.md`);
  ...
  throw new Error(`Unable to find unused patch-note path for ${slug}`);
}
...
export async function writeFetchedPatchNote({ rootDir = process.cwd(), note }) {
  ...
  const matchingExisting = existing.find(({ content }) => frontmatterValue(content, "sourceHash") === sourceHash);

  if (matchingExisting) {
    return { action: "skipped", filePath: matchingExisting.filePath };
  }
  ...
  const filePath = hasChangedExisting
    ...
    : await unusedPatchNotePath(inboxDir, slug);
