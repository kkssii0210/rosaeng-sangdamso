...
function slugForNote(note) {
  const publishedAt = String(note.publishedAt || "");

  if (/^\d{4}-\d{2}-\d{2}$/.test(publishedAt)) {
    return `${publishedAt}-update`;
  }
  ...
  return `unknown-${month}-${day}-update`;
}
...
export async function writeFetchedPatchNote({ rootDir = process.cwd(), note }) {
  ...
  const filePath = path.join(inboxDir, `${slugForNote(note)}${suffix}.md`);
