...
const MIN_SCORE = 4;
const PATCH_TERMS = new Set(["패치", "업데이트", "없데이트"]);
const CURRENT_TERMS = new Set(["이번", "이번주", "이번 주"]);
...
function compareRankedReferences(left, right) {
  if (right.score !== left.score) {
    return right.score - left.score;
  }

  const rightTime = Date.parse(right.chunk.publishedAt);
  const leftTime = Date.parse(left.chunk.publishedAt);
  if (Number.isFinite(rightTime) && Number.isFinite(leftTime) && rightTime !== leftTime) {
    return rightTime - leftTime;
  }

  const titleOrder = left.chunk.title.localeCompare(right.chunk.title);
  if (titleOrder !== 0) {
    return titleOrder;
  }

  const pathOrder = left.chunk.relativePath.localeCompare(right.chunk.relativePath);
  if (pathOrder !== 0) {
    return pathOrder;
  }

  return left.chunk.sectionTitle.localeCompare(right.chunk.sectionTitle);
}
...
export async function retrieveSgguReferences({
  ...
  const wantsCurrentPatch = includesAny(message, PATCH_TERMS) && includesAny(message, CURRENT_TERMS);
