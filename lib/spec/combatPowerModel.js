...
const ARK_GRID_ATTACK_CORE_EFFECTS = {
  relic: {
    flat: { 10: 900, 14: 900, 17: 2700, 18: 2700, 19: 2700, 20: 2700 },
    percent: { 10: 0, 14: 0.55, 17: 1.65, 18: 1.81, 19: 1.97, 20: 2.13 }
  },
  ancient: {
    flat: { 10: 1300, 14: 1300, 17: 5200, 18: 5200, 19: 5200, 20: 5200 },
    percent: { 10: 0, 14: 0.75, 17: 3, 18: 3.23, 19: 3.46, 20: 3.69 }
  }
};
...
function buildAttackPowerModel({
  ...
  const preferEquipmentFormula = attackPowerBasis === "equipment";
  const selectedBaseAttackPower = preferEquipmentFormula
    ? equipmentFormulaBaseAttackPower || profileBaseAttackBeforeBasicPercent || null
    : profileBaseAttackBeforeBasicPercent || equipmentFormulaBaseAttackPower || null;
  const selectedBaseAttackSource = preferEquipmentFormula && equipmentFormulaBaseAttackPower
    ? "equipmentFormula"
    : profileBaseAttackBeforeBasicPercent
      ? "profileBasicAttackReverse"
      : equipmentFormulaBaseAttackPower
        ? "equipmentFormula"
        : "missing";
  const selectedBasicAttackPower = selectedBaseAttackPower
  ...
  return {
    ...
    SelectedBasicAttackPower: selectedBasicAttackPower,
    AttackBreakdown: {
...
function sumAttackPowerContextPercent(sources) {
  return sources
    .filter((source) => source.SourceKind !== "flat")
    .reduce((total, source) => total + source.Percent, 0);
}

function attackPowerFactor(equipment, selectedBasicAttackPower) {
  const rawSources = collectAccessoryAttackPowerSources(equipment);
  const flatAttackPower = rawSources.reduce((total, source) => total + toNumber(source.RawValue, 0), 0);
  const rawPercent = sumAttackPowerContextPercent(rawSources);
  const percent = selectedBasicAttackPower && flatAttackPower > 0
    ? (((selectedBasicAttackPower + flatAttackPower) * (1 + rawPercent / 100)) / selectedBasicAttackPower - 1) * 100
    : rawSources.reduce((total, source) => total + source.Percent, 0);

  const sources = rawSources.map((source) => {
    if (source.SourceKind !== "flat" || !selectedBasicAttackPower) {
      return source;
    }

    return {
      ...source,
      Percent: (source.RawValue / selectedBasicAttackPower) * 100
    };
  });
  ...
  return {
    ...
    FlatAttackPower: flatAttackPower || null,
    SelectedBasicAttackPower: selectedBasicAttackPower || null,
    Sources: sources,
    Confidence: "medium"
  };
}
...
function arkGridAttackCoreThreshold(info, threshold) {
  return info.alignment === "혼돈"
    && info.shape === "별"
    && info.optionName === "공격"
    && info.point >= threshold;
}

function arkGridAttackCoreSourcesFromTooltip(slot, info) {
  const sources = [];

  collectText(valueOf(slot, ["Tooltip", "ToolTip", "tooltip"], "")).forEach((line) => {
    const text = cleanText(line);
    const point = toNumber(text.match(/\[(?<value>\d+)P\]/)?.groups?.value, 0);

    if (!arkGridAttackCoreThreshold(info, point) || /무기\s*공격력/.test(text)) {
      return;
    }

    const percentMatch = text.match(/공격력이\s*(?<value>\d+(?:\.\d+)?)%\s*증가/);
    const flatMatch = text.match(/공격력이\s*(?<value>\d[\d,]*)\s*증가/);
    const extraFlatMatch = text.match(/추가로\s*(?<value>\d[\d,]*)\s*증가/);
    const percent = toNumber(percentMatch?.groups?.value, 0);
    const flatValue = toNumber(flatMatch?.groups?.value, 0);
    const extraFlatValue = toNumber(extraFlatMatch?.groups?.value, 0);

    if (percent > 0) {
      sources.push({
        SourceType: "arkGridCore",
        SourceKind: "percent",
    ...
    [flatValue, extraFlatValue].filter((value) => value > 0).forEach((value) => {
      sources.push({
        SourceType: "arkGridCore",
        SourceKind: "flat",
        SourceName: info.name,
...
function arkGridAttackCoreSourcesFromTable(info) {
  const flatValue = arkGridPointPercent(ARK_GRID_ATTACK_CORE_EFFECTS[info.gradeKey]?.flat, info.point);
  const percent = arkGridPointPercent(ARK_GRID_ATTACK_CORE_EFFECTS[info.gradeKey]?.percent, info.point);
  const sources = [];
...
function arkGridAttackCoreSources(arkGrid) {
  return listOf(arkGrid, ["Slots", "slots"]).flatMap((slot) => {
    const info = arkGridCoreInfo(slot);

    if (!info || info.alignment !== "혼돈" || info.shape !== "별" || info.optionName !== "공격") {
      return [];
    }

    const parsedSources = arkGridAttackCoreSourcesFromTooltip(slot, info);

    return parsedSources.length ? parsedSources : arkGridAttackCoreSourcesFromTable(info);
  });
}
...
function arkGridGemEffectPercent(kind, value, contextPercent = 0) {
  if (kind === "attackPower") {
    const base = 1 + contextPercent / 100;

    return ((1 + (contextPercent + value) / 100) / base - 1) * 100;
  }
...
function arkGridGemFactors(arkGridGemEntryList, context = {}) {
  const attackPowerContextPercent = toNumber(valueOf(context, ["AttackPowerContextPercent", "attackPowerContextPercent"], 0));
  const attackPowerContextSources = listOf(context, ["AttackPowerContextSources", "attackPowerContextSources"]);

  return arkGridGemEntryList
    .filter((entry) => entry.Kind === "attackPower" || entry.Kind === "weaponPower" || entry.Kind === "bossDamage")
    .map((entry) => {
      const percent = arkGridGemEffectPercent(entry.Kind, entry.RawPercent, attackPowerContextPercent);
      ...
      return {
        ...
        Text: entry.Text,
        ...(entry.Kind === "attackPower"
          ? {
              AttackPowerContextPercent: round(attackPowerContextPercent, 5),
              AttackPowerContextSources: attackPowerContextSources
            }
          : {}),
...
function buildMissingInputs({ baseAttackPower, baseAttackSource, arkPassive, arkGrid, paradisePower, engravings }) {
  ...
  if (!listOf(arkGrid, ["Slots", "slots"]).length) {
    ...
    missing.push("아크그리드 코어는 질서/혼돈 10/14/17/18/19/20P 표 반영. 젬 공격력은 장신구와 공격 코어의 공격력% context에서 marginal 계산하고, 젬 추가 피해/보스 피해는 확정 bucket에 반영. 젬 무기 공격력은 별도 근사 계수로 반영.");
  }
...
export function buildCombatPowerAnalysis({
  ...
  const accessoryAttackPowerSources = collectAccessoryAttackPowerSources(equipment);
  const arkGridAttackCoreSourceList = arkGridAttackCoreSources(arkGrid);
  const attackPowerContextSources = [
    ...accessoryAttackPowerSources.filter((source) => source.SourceKind !== "flat"),
    ...arkGridAttackCoreSourceList.filter((source) => source.SourceKind !== "flat")
  ];
  const attackPowerPercentContext = sumAttackPowerContextPercent(attackPowerContextSources);
  ...
  const factors = [
    ...
    attackPowerFactor(equipment, attackModel.SelectedBasicAttackPower),
    ...
    ...arkGridGemFactors(arkGridGemEntryList, {
      AttackPowerContextPercent: attackPowerPercentContext,
      AttackPowerContextSources: attackPowerContextSources
    })
