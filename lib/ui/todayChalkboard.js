...
function topCandidateOf(specUpRecommendation) {
  const candidates = specUpRecommendation?.Recommendation?.TopCandidates || specUpRecommendation?.TopCandidates;
...
function titleForCandidate(candidate) {
  ...
  const type = String(candidate?.Type || candidate?.type || "");

  if (type === "gem" || /보석|겁화|작열|멸화|홍염/.test(label)) {
    return "오늘은 보석부터 보는 게 좋겠습니다.";
  }

  if (type === "accessory" || /악세|목걸이|귀걸이|반지/.test(label)) {
    return "악세 교체를 먼저 비교해봅시다.";
  }

  if (["enhancement", "weaponHoning", "armorHoning"].includes(type) || /강화|무기|방어구/.test(label)) {
...
export function buildTodayChalkboardState(input = {}) {
  const {
    status = "idle",
    armory = null,
